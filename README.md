...
第一段階はフラグメント原文の保存・分類別表示・フラグメントの実装状態管理。
専用LLM相談パネル、Ccからの自動登録、レコンストラクション、構造化仕様の実装状態管理は後続で実装する。
...
仕様タブの「可視化」で、構造化仕様を状態・分類・優先度のいずれかの軸でグループ分けした図として見られます。
グループごとに表示を切り替え、その図を Tela 用ファイル (`TELA_SPEC_VIEW 1`) へ書き出せます。詳細:
[仕様書可視化ビュー](spec/feature/spec-visualization-view.md)。
