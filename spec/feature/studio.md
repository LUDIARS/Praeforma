...
- Studio は `projects.anatomia_repo` で指定した Anatomia Web API project を直接参照する。
  `GET /api/graph?project=` と、ドメイン対象時の
  `GET /api/projects/:id/domain-view` を呼び、コードグラフを Praeforma スキーマへ正規化する。
- LLM は **claude CLI (`claude -p`)** を spawn (LUDIARS 規約: API 不使用)。 `lib/llm.ts`。
- 設定不備 (Anatomia URL 未設定 / claude 不在) は **mock に落とさず明示エラー** (無言 fallback 禁止)。
...
④ Anatomia ─ 関連処理を直接検索 → code graph を upsert
...
| POST | `/anatomia-link` | Anatomia 直接取得 → グラフ upsert + run 記録 | owner/planner |
...
対象ドメインの実装 AnchorId と検索語（2 文字以上のトークンで node の name/path を照合）を
和集合し、選択ノード間の edge のみを残す。`file` は `node.type=file`、その他は `symbol`、
edge は calls/depends/implements/related に正規化する。ドメイン未検出・検索結果なしは空グラフと
理由を返し、上流未設定・到達不能・HTTP 失敗は明示エラーにする。
...
| `PRAEFORMA_ANATOMIA_URL` | (なし) | Anatomia Web API base URL。未設定なら `/anatomia-link` は 503 `anatomia_unconfigured` |
| `PRAEFORMA_ANATOMIA_TOKEN` | (なし) | Anatomia API の bearer token（任意） |

プロジェクトごとの Anatomia project id は `projects.anatomia_repo` に設定する。未設定なら
`/anatomia-link` は 409 `anatomia_repo_unset` を返す。
