...
3. Genius から類似判断カードを検索する。LLM は各カードが各候補へ適用可能、不適用、未確定の
   ...
   **この手順はオプトイン。** `PRAEFORMA_GENIUS_URL` を設定したときだけ実行する。
...
5. 人間判断後に限り、その判断を Genius へカードとして送る。送信の失敗は判断自体を取り消さず、
   ...
   **この手順もオプトイン。** 無効時は送出を試みず `geniusPublishStatus=not_requested` のまま
   残す。`publishing` を経由しないのは、一度も送らなかった判断を「送って落ちた」と区別できなく
   しないため。

### Genius 連携はオプトイン

Genius は判断カードの蓄積先であって、UX・コアドメイン設計が成立するための必須依存ではない。
過去判断の再利用は「あれば効く」補助で、境界の採否は常に人間が行う。未設定を失敗として扱うと、
補助が無いだけで設計作業そのものが止まる。

無効時は手順 3 を飛ばし、カード 0 件として手順 4 へ進む。カードが無いときは
「再利用できる判断が無いので、人間が境界の採否理由を記録する」という human question が立ち、
設計フロー自体はそのまま成立する。
