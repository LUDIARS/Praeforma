...
2026-09-08: ScreenFlowはプロジェクト上部の「UXデザイン」の横から開く。下段の遷移図は既存のFlowDiagramを共有し、プロジェクトの共通タブ「遷移図」からも単独表示できる。グループ化・Mermaidソース・ダウンロードを両方で維持する。旧ScreenFlowの3ペインと下段タブは残す。
