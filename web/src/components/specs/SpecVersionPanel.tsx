...
export function SpecVersionPanel({pid}:{pid:string}):React.ReactElement {
  ...
  const refresh=async()=>{await Promise.all(['spec-versions','spec-reconstruction','specs','spec-fragments','domain-definitions'].map(key=>client.invalidateQueries({queryKey:[key,pid]})));};
  ...
  return <section className="spec-version-panel"><h4>仕様バージョン {query.data?.version??'…'}</h4>
    <p className="meta">追加でパッチ更新、レコンストラクション確定でマイナー更新、リリース記録でメジャー更新。</p>
    <DomainProposalPanel key={pid} pid={pid}/>
    {query.data?.canEdit?<div className="simple-actions"><button type="button" disabled={busy} onClick={()=>{confirm.reset();release.reset();generate.mutate();}}>{generate.isPending?'統合案を作成中…':'レコンストラクション'}</button><button type="button" className="ghost" disabled={busy} onClick={()=>setShowRelease(!showRelease)}>リリースを記録</button></div>:null}
