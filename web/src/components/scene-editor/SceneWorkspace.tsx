...
export function SceneWorkspace({pid,lid,initial,canEdit}:{pid:string;lid:string;initial:SceneDocument;canEdit:boolean}):React.ReactElement {
  ...
  const [saved,setSaved]=React.useState(JSON.stringify(initial.canvas));const [showImport,setShowImport]=React.useState(false);
  const [showReferences,setShowReferences]=React.useState(true);const [importError,setImportError]=React.useState('');
  const dirty=JSON.stringify(history.canvas)!==saved;
  React.useEffect(()=>{
    if(!dirty)return;
  ...
  const save=useMutation({mutationFn:()=>sceneApi.save(pid,lid,{canvas:history.canvas,sources:sources.filter(source=>history.canvas.frames.some(frame=>frame.id===source.frameId))}),
    onSuccess:({document})=>{history.reset(document.canvas);setSources(document.sources);setSaved(JSON.stringify(document.canvas));}});
