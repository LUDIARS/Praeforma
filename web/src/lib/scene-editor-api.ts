...
export const sceneApi={
  ...
  save:(pid:string,lid:string,document:SceneDocument)=>{
    ...
    return req<{document:SceneDocument}>(root(pid,lid),{method:'PUT',body:JSON.stringify({canvas:{...canvas,expected_revision:revision},sources:document.sources,web:document.web,layers:document.layers})});
  },
